<rule_activation id="d6745fb3-0837-49f4-ab40-f6598046de09" title="Adopt Event-Driven Logging Pattern for Asynchronous Operations: Event Logs Include" applies_to="**/*">
...
- **R-EDL-001** MUST: Event logs MUST include contextual metadata such as event type, timestamp, and correlation identifiers to enable tracing across asynchronous boundaries.
...
- Document the logging pattern with code examples from detected files (launcher.ts, scrcpy-stream.ts, io-client.ts) as reference implementations

<enforcement>
Claude Code MUST verify event-driven logging compliance before approving changes to event-driven components. Verification is mandatory and cannot be deferred.
</enforcement>