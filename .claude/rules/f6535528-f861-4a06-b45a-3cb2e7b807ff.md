<rule_activation id="f6535528-f861-4a06-b45a-3cb2e7b807ff" title="Standardize Middleware-Based Logging for Request/Response Lifecycle: Middleware Logging Capture" applies_to="**/*middleware*.ts,**/*Middleware*.ts,**/baseDB.ts,**/server.ts,**/launcher.ts">
...
- **R-MIDDLEWARE-001** MUST: Middleware logging MUST capture request identifiers, timestamps, operation type, and execution duration to enable request tracing and performance analysis.
...
- Middleware logging uses JSON structured format with standard fields: timestamp, requestId, operation, duration, status, and error
- Correlation IDs are propagated through middleware layers for end-to-end request tracing

<enforcement>
Claude Code MUST verify middleware logging compliance before approving changes to middleware components. Verification is mandatory and MUST NOT be skipped or deferred. CI/CD pipeline MUST fail if new middleware components are detected without logging implementation.
</enforcement>