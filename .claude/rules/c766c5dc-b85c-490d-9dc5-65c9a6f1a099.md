<rule_activation id="c766c5dc-b85c-490d-9dc5-65c9a6f1a099" title="Adopt Event-Driven Process Lifecycle Management for External Services: External Process Integrations" applies_to="**/*">
...
- **R-EDPLM-001** MUST: External process integrations MUST implement event-driven lifecycle management with explicit startup, ready, and shutdown phases.
...
```bash
# Check for lifecycle management patterns (spawn/exec with event handlers)
...
<enforcement>
Verification by code review checklist requiring lifecycle management verification for any PR touching external process integration.
Verification by CI pipeline checks for process leaks after test suite execution.
Verification by static analysis rules detecting spawn/exec calls without corresponding cleanup handlers.
Verification by integration test coverage requirements for startup and shutdown paths.
...
Exception process: Submit exception request to tech lead with justification and risk assessment; document in code comments with reference to approval and timeline; add technical debt ticket for future remediation if temporary; review exceptions quarterly.
