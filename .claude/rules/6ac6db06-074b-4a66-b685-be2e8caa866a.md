<rule_activation id="6ac6db06-074b-4a66-b685-be2e8caa866a" title="Adopt Event-Driven Logging Pattern for Asynchronous Operations: Components Implement Structured" applies_to="**/*">
...
- **R-EDL-001** MAY: Components MAY implement structured logging formats (JSON, key-value pairs) to facilitate automated log analysis and aggregation.
...
# Run linting and logging-specific tests
...
<enforcement>
Claude Code MUST verify compliance with this rule during code review. Violations require logging at event boundaries before approval. Static analysis tools MUST be configured to detect event emitters/listeners without associated logging. Integration tests MUST verify log output for critical event flows. Exceptions require performance profiling data or technical justification reviewed by architecture review board.
</enforcement>