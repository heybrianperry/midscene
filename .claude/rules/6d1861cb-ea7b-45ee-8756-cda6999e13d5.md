<rule_activation id="6d1861cb-ea7b-45ee-8756-cda6999e13d5" title="Standardize Browser and Platform API Abstraction Layers for External Integration: Components Consuming Platform" applies_to="**/*">
...
- **R-PLAT-001** SHOULD: Components consuming platform APIs SHOULD depend on abstraction interfaces rather than concrete implementations to enable testing with mocks or stubs.
...
```bash
# Check for direct Chrome API references outside abstraction modules
...
# Check for direct process API references outside abstraction modules
...
<enforcement>
Claude Code MUST verify all platform API usage against these rules. Violations MUST be flagged during code review. CI pipeline MUST fail if direct platform API usage is detected outside approved abstraction modules. Pull requests MUST be blocked until platform API access is properly abstracted. Existing violations MUST be tracked as technical debt items with remediation timeline.
</enforcement>