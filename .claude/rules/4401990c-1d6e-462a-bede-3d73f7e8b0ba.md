<rule_activation id="4401990c-1d6e-462a-bede-3d73f7e8b0ba" title="Standardize Test File Naming with .test.ts Extension for TypeScript Tests: Test Files Organized" applies_to="**/*.test.ts">
...
- **R-TST-001** MAY: Test files MAY be organized in dedicated test directories (e.g., tests/, __tests__/) or co-located with source files based on package conventions.
...
<enforcement>
Claude Code MUST NOT skip or defer verification of test file naming conventions. All TypeScript test files MUST use the .test.ts extension unless explicitly documented as an approved exception.
</enforcement>