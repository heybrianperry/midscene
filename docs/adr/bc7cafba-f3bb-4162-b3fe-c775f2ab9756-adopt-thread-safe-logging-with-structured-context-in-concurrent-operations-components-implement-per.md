# Adopt Thread-Safe Logging with Structured Context in Concurrent Operations: Components Implement Per
...
1. MAY: Components MAY implement per-thread or per-worker log buffers that are periodically flushed to a central logging sink

## Policy Block

- MAY Components MAY implement per-thread or per-worker log buffers that are periodically flushed to a central logging sink
