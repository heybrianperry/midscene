# Standardize Test Organization in Dedicated Test Directories for CI/CD Integration: Test File Names
...
1. SHOULD: Test file names SHOULD mirror the functionality being tested (e.g., server-mjpeg-stream.test.ts, http-server.test.ts) to improve discoverability

## Policy Block

- SHOULD Test file names SHOULD mirror the functionality being tested (e.g., server-mjpeg-stream.test.ts, http-server.test.ts) to improve discoverability
