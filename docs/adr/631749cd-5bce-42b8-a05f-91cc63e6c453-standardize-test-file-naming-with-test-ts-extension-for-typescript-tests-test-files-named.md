# Standardize Test File Naming with .test.ts Extension for TypeScript Tests: Test Files Named
...
1. MUST: Test files MUST be named with a descriptive prefix followed by .test.ts (e.g., batch-runner.test.ts, serialize-zod-field.test.ts)

## Policy Block

- MUST Test files MUST be named with a descriptive prefix followed by .test.ts (e.g., batch-runner.test.ts, serialize-zod-field.test.ts)
