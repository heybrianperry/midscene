# Adopt Thread-Safe Logging with Structured Context in Concurrent Operations: Log Entries Generated
...
1. MUST: Log entries generated from concurrent operations MUST include structured context identifiers (worker ID, bridge ID, request ID, or correlation ID) to enable tracing across execution boundaries

## Policy Block

- MUST Log entries generated from concurrent operations MUST include structured context identifiers (worker ID, bridge ID, request ID, or correlation ID) to enable tracing across execution boundaries
