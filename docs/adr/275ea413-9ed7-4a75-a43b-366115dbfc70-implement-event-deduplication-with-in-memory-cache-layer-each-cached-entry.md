# Implement Event Deduplication with In-Memory Cache Layer: Each Cached Entry
...
1. MUST: Each cached entry MUST have a unique identifier (event ID, message hash, or composite key) that can be used for deduplication lookups

## Policy Block

- MUST Each cached entry MUST have a unique identifier (event ID, message hash, or composite key) that can be used for deduplication lookups
